# Weather App

Built with React and Vite. HTML, CSS, and JavaScript are included.

## Quick View

[View Weather App On Vercel](https://weather-app-xi-silk-31.vercel.app)

## Features

- Responsive design
- Anchor links for smooth navigation
- Location-based
- Uses Open Weather Map's API
- Screen reader accessibility

## Preview

<img src="screenshots/Weather-App.png" width="60%" alt="Weather App" />

## Getting Started

1. Clone the repo:
   ```bash
   git clone https://github.com/SilentViewer807/Weather-App.git
   ```
2. Navigate to the project directory:
   ```bash
   cd Weather-App
   ```
3. Install dependencies:
   ```bash
   npm install
   ```
4. Set up environment variables:
   * If you don't see `.env.example` in the root directory, follow these substeps:
        * On macOS: Press ```Cmd + Shift + .``` while in the folder.
        * On Windows: Depending on your version, click the View tab, and show hidden files.
   * Rename `.env.example` in the root directory to `.env`.
   * Add your OpenWeather API key inside the file:
     ```env
     VITE_OPENWEATHER_API_KEY=your_own_api_key
     ```
6. Run the development server:
   ```bash
   npm run dev
   ```

## License

This project is open source and available under the MIT License.
